...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const GUTTER = 32;
		/** One-click sizes. `max` deliberately overflows, so min() clamps it to the viewport. */
		const PRESETS = [
			{ id: "std", label: "标准 800×800", w: 800, h: 800 },
			{ id: "lg", label: "大 1080×900", w: 1080, h: 900 },
			{ id: "xl", label: "超大 1400×1080", w: 1400, h: 1080 },
			{ id: "max", label: "近全屏", w: 3000, h: 2000 }
		];
		...
		const ui = {
			...
			chip: {
				...
				// The border is the selection indicator, so an unselected preset
				// draws none. It stays 1px and transparent rather than 0 so
				// selecting one never reflows the row.
			...
			chipActive: {
				// `border`, not `borderColor`: React must see the SAME property
				// change in both directions. Spreading a `borderColor` longhand
				// over the `border` shorthand works while activating, but on
				// deactivate React clears the longhand alone — the shorthand has
				// already expanded into border-width/style, so the colour falls
				// back to `currentColor` and every clicked preset keeps a border.
		...
		function SettingsSizeRow() {
			...
			return React.createElement("div", {
				...
				style: ui.title
			}, "设置弹框尺寸"), React.createElement("div", {
				style: ui.grid
			}, PRESETS.map((preset) => {
			...
			})), sliderRow("宽度", size.w, MIN_W, MAX_W, (value) => setSize(value, size.h)), sliderRow("高度", size.h, MIN_H, MAX_H, (value) => setSize(size.w, value)), React.createElement("div", {
				style: ui.hint
			}, "当前 " + size.w + " × " + size.h + " px · 超出屏幕时自动收窄 · 仅保存在本浏览器"), React.createElement("div", {
				style: ui.grid
			}, React.createElement("button", {
				type: "button",
				onClick: () => setSize(DEFAULT_SIZE.w, DEFAULT_SIZE.h),
